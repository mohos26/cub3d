...
/*   Updated: 2025/07/03 14:09:54 by aouanni          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	hiro_intersec_check(t_game *data, int map_x, int map_y)
{
		...
		&& data->ray.next_y >= 0 && data->ray.next_y < data->map_height * TILE)
	{
		...
		if (map_y >= 0 && map_x >= 0 && map_y < data->map_height
			&& map_x < data->map_width && is_wall(map_x, map_y, data))
		{
			is_wall_found = 1;
...
void	hiro_intersec(double r_angle, t_game *data)
{
	...
	data->ray.x_intersec = data->player.x
		+ (data->ray.y_intersec - data->player.y) / tan(r_angle);
	data->ray.y_step = TILE;
	if (data->ray.face_up)
		data->ray.y_step *= -1;
	data->ray.x_step = TILE / tan(r_angle);
	if ((data->ray.face_right && data->ray.x_step < 0)
		|| (data->ray.face_left && data->ray.x_step > 0))
		data->ray.x_step *= -1;
...
void	vert_intersec(double r_angle, t_game *data)
{
	...
	data->ray.y_intersec = data->player.y
		+ ((data->ray.x_intersec - data->player.x) * tan(r_angle));
	data->ray.x_step = TILE;
	if (data->ray.face_left)
		data->ray.x_step *= -1;
	data->ray.y_step = TILE * tan(r_angle);
	if ((data->ray.face_up && data->ray.y_step > 0)
		|| (data->ray.face_down && data->ray.x_step < 0))
		data->ray.y_step *= -1;
	data->ray.next_x = data->ray.x_intersec;
	data->ray.next_y = data->ray.y_intersec;
